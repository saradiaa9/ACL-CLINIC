...
import { Box, Text, Button, Table, Thead, Tbody, Tr, Th, Td, Input, VStack, HStack, Center } from "@chakra-ui/react";
...
function Documents() {
  ...
  const upload = async (e) => {
    ...
    try {
      await axios.post(`/Patient/upload?username=${username}`, formData);
  ...
  const removeDocument = async (filename) => {
    try {
      ...
      await axios.post(`/Patient/removeDocuments?username=${username}&filename=${filename}`);
  ...
  const fetchDocuments = async () => {
    try {
      const response = await axios.get(`/Patient/getDocuments?username=${username}`);
      ...
      <Center>
        <Box p={8} borderWidth={1} borderRadius={8} boxShadow="lg">
          ...
          {documents.length > 0 ? (
            ...
            <Text>No documents found for {username}</Text>
          )}
        </Box>
      </Center>
    </VStack>
